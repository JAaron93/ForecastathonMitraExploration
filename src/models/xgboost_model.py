...
class XGBoostModel(BaseModel):
        ...
        **kwargs,
        ...
        if optimize:
            ...
            best_params = self.optimize_hyperparameters(
                X, y, params=optimization_params or {}
            )
        ...
        self.model_object = model_cls(objective=self.objective, **self.hyperparameters)
        ...
        # Fit model
        self.model_object.fit(X, y, eval_set=eval_set, verbose=False, **kwargs)
        self.is_fitted = True
    ...
    def optimize_hyperparameters(
        ...
        if metric not in ["rmse", "accuracy", "logloss"]:
            logger.warning(
                f"Unknown metric '{metric}' provided. Falling back to 'rmse'."
            )
            metric = "rmse"
        ...
        def objective(trial):
            ...
            for train_idx, val_idx in tscv.split(X):
                ...
                model = model_cls(
                    **param, n_jobs=1
                )  # Single job per trial to avoid oversubscription
