...
class LSTMModel(BaseModel):
    ...
    def from_config(cls, config: Dict[str, Any]) -> "LSTMModel":
        """Create model instance from config with backward compatibility."""
        # Handle both old and new model type identifiers
        model_type = config.get("model_type", "").lower()
        if model_type not in ("lstm", "lstm_keras"):
            raise ValueError(f"Unexpected model type for LSTMModel: {model_type}")
    ...
    def _create_sequences(
        self, X: np.ndarray, y: Optional[np.ndarray] = None, seq_len: int = 10
        ...
        if n_samples <= seq_len:
            raise ValueError(
                f"Input data length ({n_samples}) must be greater than sequence length ({seq_len})"
            )
    ...
    def _build_model(self, input_shape: Tuple[int, int], output_shape: int):
        ...
        for i in range(num_layers):
            ...
            model.add(
                layers.LSTM(
                    hidden_size, return_sequences=return_sequences, dropout=dropout
                )
            )
    ...
    def predict(self, X: pd.DataFrame) -> np.ndarray:
        ...
        if len(X_np) <= seq_len:
            raise ValueError(
                f"Input length {len(X_np)} too short for sequence length {seq_len}"
            )
    ...
    def load_model(self, path: str) -> "LSTMModel":
        ...
        # Backward compatibility: old serialized models may have "lstm_keras" as model_type
        model_type = metadata.get("model_type", "").lower()
        if model_type not in ("lstm", "lstm_keras"):
            logger.warning(
                f"Unexpected model type '{model_type}' in metadata, proceeding anyway"
            )
