...
def test_analyze_robustness_metadata_inference(comparator):
    ...
    class FallbackModel(BaseModel):
        @property
        def model_type(self): return "xgboost"
        def fit(self, X, y, **kwargs): return self
        def predict(self, X): return np.array([0.1, 0.5, 0.9])
    ...
    results = comparator.analyze_robustness(X, y_clf) # Pass discrete y but metadata says regression
    assert "rmse" in results["global"].columns
    ...
    class StrictModel(BaseModel):
        def __init__(self, **kwargs):
            if "invalid" in kwargs:
                raise TypeError("__init__() got an unexpected keyword argument 'invalid'")
        @property
        def model_type(self) -> str: return "strict"
        def fit(self, X, y, **kwargs): return self
...
def test_instantiate_model_type_error_missing_kwargs():
    ...
    class MissingKwargModel(BaseModel):
        def __init__(self, required_arg, **kwargs):
            pass
        @property
        def model_type(self) -> str: return "missing"
        def fit(self, X, y, **kwargs): return self
        def predict(self, X): return np.zeros(len(X))
        def predict_proba(self, X): return np.zeros((len(X), 2))
        def get_feature_importance(self): return {}
