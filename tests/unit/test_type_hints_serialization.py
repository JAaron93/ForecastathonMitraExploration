...
class TestJSONFunctions:
    ...
    def test_save_json_union_types(self, tmp_path):
        ...
        test_data = {"mixed": [1, "two", 3.0, True], "nested": {"a": 1, "b": [2, 3]}}
        path = tmp_path / "test.json"
        save_json(test_data, path)
        assert path.exists()
    ...
    def test_invalid_json_data(self, tmp_path):
        ...
        with pytest.raises(TypeError):
            save_json({1, 2, 3}, tmp_path / "test.json")
...
class TestTimeSeriesDataFunctions:
    ...
    def create_test_timeseries_data(self):
        ...
        features_data = {"feature1": range(5), "feature2": range(5, 10)}
...
class TestErrorHandling:
    ...
    def test_file_permissions(self, tmp_path):
        """Test handling of file permission issues, skipping on non-POSIX systems."""
