...
def config_and_path(draw):
    config = draw(
        st.dictionaries(st.text(min_size=1), json_values, min_size=1, max_size=5)
    )
...
class TestConfigManagerProperties:
    ...
    def test_set_value_consistency(self, config, path, value):
        ...
        retrieved = self.manager.get_value(new_config, path)

        # Verify the new config has the updated value
        assert json.dumps(retrieved, sort_keys=True) == json.dumps(
    ...
    def test_validation_error_message(self):
        ...
        schema = {
            ...
            "properties": {
                "level1": {
                    "type": "object",
                    "properties": {"level2": {"type": "integer"}},
                }
            },
        }
